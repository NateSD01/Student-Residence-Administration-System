...
$(document).ready(function () {
    ...
    function performSearch() {
		...
		// Remove any existing "No FAQs found" message
		...
		if (!foundResults) {
			...
			//$('.panel-group').append('<p>No FAQs found.</p>');
				$('.panel-group').append('<p id="noResultsMessage">No FAQs found.</p>');
				$('#goBackButton').show();
            } else {
                // Remove the "No FAQs found" message if it exists
                //$('.panel-group p:contains("No FAQs found.")').remove();
...
<?php
try {
    $stmt = $pdo->prepare("SELECT * FROM faqs"); // Replace 'faq_table' with your actual table name
    $stmt->execute();
...
<!--end of added search bar-->
		...
		<!--added code to search greater than 0-->
		<?php if (count($faqs) > 0): ?>
		<!--end of added code to search greater than 0-->
			...
			<!--added code for not found faq-->
			<!--p>No FAQs found.</p-->
            <?php endif; ?>
			<!---end of code for not found faq-->
			 <!-- Add the "No FAQs found" message with an ID to hide it initially -->
			 <p id="noResultsMessage" style="display: none;">No FAQs found.</p>
		</div>
	</div>
</div>
...
<style>
	...
	body 
	{
   /*background-color: #000000; /* Dark color of your choice */
	background-image: url('residencefullview.jpeg');
    margin: 0;
    padding: 0;
	}
